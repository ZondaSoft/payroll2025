...
; (function ($, undefined) {
    ...
    function miCallback(resultado) {
        if (resultado) {
            ...
            if (window.location.pathname === '/sicoss/importar') {
                fetch("/sicoss/import-resultados")
                    ...
                    .then(data => {
                        ...
                        btnOk.onclick = () => {
                            window.location.href = "/recibos/exportar-ok";
                        };
                        ...
                        btnErr.onclick = () => {
                            window.location.href = "/sicoss/importar/exportar-err";
                        };
            ...
            if (window.location.pathname === '/novedadesind/importar') {
                fetch("/recibos/import-resultados")
                    .then(res => res.json())
                    .then(data => {
                        // Actualizar contadores en el DOM
                        ...
                        btnOk.removeAttribute('hidden');
                        btnOk.onclick = () => {
                            window.location.href = "/sicoss/importar/exportar-ok";
                        };
            ...
            document.getElementById('registrosErr').innerText =
                'Se detectó un error desconocido en la importación';
        }
    ...
    function finishStep(wizard, state) {
        ...
        if (wizard.triggerHandler("finishing", [state.currentIndex])) {
            ...
            if (window.location.pathname === '/recibos/add') {
                window.location.href = '/recibos'; // Redirige
                // Mostrar mensaje
                document.getElementById('mensaje-procesando').style.display = 'block';
            ...
            if (window.location.pathname === '/novedadesind/importar') {
                window.location.href = '/novedadesind'; // Redirige
                // Mostrar mensaje
                document.getElementById('mensaje-procesando').style.display = 'block';

                // Esperar 1 segundo y redirigir
                setTimeout(function () {
                    window.location.href = '/novedadesind';
                }, 100);
            }
        }
        else {
    ...
    function goToNextStep(wizard, options, state) {
        ...
        if (index == 1) {
            // Inhabilito botones
            $(".actions a[href$='#previous']").addClass("disabled");
            $(".actions a[href$='#finish']").addClass("disabled");
            ...
            // Llamamos a subirArchivo pasándole el callback
            subirArchivo(miCallback);
        }
        //}
    ...
    function goToPreviousStep(wizard, options, state) {
        ...
        if (index == 2) {
            ...
            // Mostrar y actualizar registros OK
            const registrosOk = document.getElementById('registrosOk');
            registrosOk.innerText = "Registros importados correctamente: ";
            registrosOk.setAttribute('hidden', true);
            ...
            // Mostrar y actualizar registros Err
            const registrosErr = document.getElementById('registrosErr');
            registrosErr.innerText = "Registros rechazados: ";
            registrosErr.setAttribute('hidden', true);

            const btnErr = document.getElementById('btn-err');
            btnErr.setAttribute('hidden', true);

            document.getElementById('titleFinish').innerText =
                'Proceso de importación en proceso';

            // Deshabilito botones del stepper
            //$(".actions a[href$='#previous']").addClass("disabled");
            $(".actions a[href$='#finish']").addClass("disabled");
        }
        //}
