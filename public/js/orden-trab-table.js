...
$(function () {
  ...
  if (dt_invoice_table.length) {
    ...
    var dt_invoice = dt_invoice_table.DataTable({
      ajax: '/orders/json',    // JSON file to add data
      columns: [
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            var $row_output = '<a href="/new-order/' + $id + '"><span># ' + $numero + '</span></a>';
            return $row_output;
          }
        },
        {
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            var $row_output =
              ...
              $name +
              '</p></a>' +
              '<small class="text-truncate">Mecanico: ' +
              $mecanico +
              '</small>' +
              '</div>' +
              '</div>';
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            return (
              ...
              '<a href="javascript:;" class="btn btn-sm btn-icon btn-text-secondary waves-effect waves-light rounded-pill delete-record" data-bs-toggle="modal" data-bs-target="#backDropModal" data-id=' + full['id'] + ' onclick="setModalId(this)" data-bs-placement="top" title="Anular orden"><i class="ri-delete-bin-7-line ri-20px"></i></a>' +
              '<a href="/new-order/' + full['id'] + '" data-bs-toggle="tooltip" class="btn btn-sm btn-icon btn-text-secondary waves-effect waves-light rounded-pill"  data-bs-placement="top" title="Ver orden de trabajo"><i class="ri-eye-line ri-20px"></i></a>' +
              '<div class="dropdown">' +
              ...
              '<a href="/new-order/edit/' + full['id'] + '" class="dropdown-item">Editar</a>' +
              '<a href="/new-order/copy/' + full['id'] + '" class="dropdown-item">Duplicar</a>' +
              '</div>' +
              '</div>' +
              '</div>'
            );
      ...
      buttons: [
        {
          text: '<i class="ri-add-line ri-16px me-md-2 align-baseline"></i><span class="d-md-inline-block d-none">Crear Nueva orden</span>',
