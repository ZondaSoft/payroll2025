...
class BajasController extends Controller
{
    public function index($id = null, $direction = null)
    {
        ...
        if ($id == null or $id == 0) {
            ...
            //------------------------------
            if ($direction == null) {
                $legajo = Sue070::find($id);
                if ($legajo == null) {
                    $legajo = Sue070::orderBy('codigo')
                        ->first();      // first(); // find($id);     // dd($legajo);   // $legajo = new Sue070;
                }
            } elseif ($direction == -1) {
                //------------------------------
                //      Registro anterior
                //------------------------------
                $legajo = Sue070::where('id', '<', $id)
                    ->orderBy('id', 'desc')
                    ->first();
     ...
     */
    public function edit(Sue070 $empleado)
    {
        return Inertia::render('Empleados/Form', [
            'empleado' => $empleado,
            'isEditing' => true,
     ...
     */
    public function update(Request $request, Sue070 $empleado)
    {
        $validated = $request->validate([
     ...
     */
    public function destroy(Sue070 $empleado)
    {
        $empleado->delete();
    ...
    public function first()
    {
        ...
        return redirect()->route('bajas.show', $legajo->id);
    }
