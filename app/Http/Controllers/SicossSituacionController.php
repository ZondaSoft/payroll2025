...
class SicossSituacionController extends Controller
{
    public function index($id = null, $direction = null)
    {
        ...
        if ($id == null or $id == 0) {
            ...
            //      Primer registro
            //------------------------------
            $legajo = Sicoss12::orderBy('codigo')
                ->first();      // find($id);
        } else {
            //------------------------------
            //      Primer registro
            //------------------------------
            if ($direction == null) {
                $legajo = Sicoss12::find($id);
                if (!$legajo) {
                    $legajo = Sicoss12::orderBy('codigo')
                        ->first();      // first(); // find($id);     // dd($legajo);   // $legajo = new Sicoss12;
                }
            } elseif ($direction == -1) {
                //------------------------------
                //      Registro anterior
                //------------------------------
                $legajo = Sicoss12::where('id', '<', $id)
                    ->orderBy('id', 'desc')
                    ->first();
    ...
    {
        return Inertia::render('Sicoss/Situacion', [
            'legajo' => new Sicoss12(),   // id null, codigo null, etc
            'agregar' => true,
            'edicion' => true,
            'active' => 64,
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            'codigo' => 'required|integer|min:0|max:999999|unique:Sicoss12s,codigo',
            'detalle' => 'required|string|max:100',
        ]);

        Sicoss12::create($validated);

        return redirect()->route('sicoss.situacion.index')
            ->with('success', 'Situación creada exitosamente.');
    }

    public function show($id, $direction = null)
    {
        if (!$id || $id == 0) {
            $legajo = Sicoss12::orderBy('codigo')->first();
        } else {
            if ($direction === null) {
                $legajo = Sicoss12::find($id);
            } elseif ($direction == -1) {
                ...
                $legajo = Sicoss12::orderBy('id', 'desc')->first();
            }
    ...
    {
        $legajo = Sicoss12::orderBy('id', 'asc')->first();
        ...
        return redirect()->route('sicoss.situacion.show', $legajo->id);
    }
    ...
    {
        $previousId = Sicoss12::where('id', '<', $id)
            ->orderBy('id', 'desc')
            ->first()?->id;

        if (!$previousId) {
            return redirect()->route('sicoss.situacion.index', $id)
                ->with('warning', 'No hay registro anterior');
        }

        return redirect()->route('sicoss.situacion.show', $previousId);
    }

    // Registro siguiente
    public function next($id)
    {
        $nextId = Sicoss12::where('id', '>', $id)
            ->orderBy('id', 'asc')
            ->first()?->id;

        if (!$nextId) {
            return redirect()->route('sicoss.situacion.index', $nextId)
                ->with('warning', 'No hay registro siguiente');
        }

        return redirect()->route('sicoss.situacion.show', $nextId);
    }

    // Búsqueda
    public function search(Request $request)
    {
        $query = Sicoss12::query();
