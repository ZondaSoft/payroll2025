...
const determineActionRoute = () => {
  if (props.agregar) {
    ...
    return '/sicoss/condiciones/add';
  } else if (props.edicion) {
    return `/sicoss/condiciones/edit/${form.id}`;
  } else {
    return '/sicoss/condiciones'; // Ruta predeterminada o de error
  }
};
...
const submit = () => {
    let ruta = 'sicoss.condiciones.update';

    if (usePage().props.agregar) {
        ruta = 'sicoss.condiciones.store';
...
const borrar = () => {
    ...
    router.delete(route('sicoss.condiciones.destroy', id), {
        preserveScroll: true,
