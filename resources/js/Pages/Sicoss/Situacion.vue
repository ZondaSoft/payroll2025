...
const determineActionRoute = () => {
  if (props.agregar) {
    ...
    return '/sicoss/situacion/add';
  } else if (props.edicion) {
    return `/sicoss/situacion/edit/${form.id}`;
  } else {
    return '/sicoss/situacion'; // Ruta predeterminada o de error
  }
};
...
const submit = () => {
    let ruta = 'sicoss.situacion.update';

    if (usePage().props.agregar) {
        ruta = 'sicoss.situacion.store';
...
const borrar = () => {
    ...
    router.delete(route('sicoss.situacion.destroy', id), {
        preserveScroll: true,
        ...
        <div class="container-xxl flex-grow-1 container-p-y">
            <div class="app-ecommerce">
                ...
                <FormHeader
                    ...
                    :edicion="edicion"
                    :form-id="form.id"
                    titulo="Sicoss - Situación de revista"
                    :ruta-create="route('sicoss.situacion.create')"
                    :ruta-edit="form.id ? route('sicoss.situacion.edit', form.id) : null"
                    :ruta-first="route('sicoss.situacion.first')"
                    :ruta-previous="form.id ? route('sicoss.situacion.previous', form.id) : null"
                    :ruta-next="form.id ? route('sicoss.situacion.next', form.id) : null"
                    :ruta-last="route('sicoss.situacion.last')"
                    :ruta-search="route('sicoss.situacion.search')"
                    :ruta-index="route('sicoss.situacion.index')"
                    :on-submit="submit"
                />
                <!-- END HEAD Y BOTONES -->
