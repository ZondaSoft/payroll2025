  ...
  <div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-6 row-gap-4">
      <div class="d-flex flex-column justify-content-center">
        <h4 class="mb-1">Importar liquidación externa</h4>
      </div>
    </div>
                                ...
                                <div class="col-md-12 pl-0 mt-5">
                                    ...
                                    <div class="row">
                                        <div class="col-lg-4 mb-2">
                                            <label class="col-form-label" for="periodo2">Periodo * </label>
                                        ...
                                        <div class="col-lg-5 mb-2">
                                            <label class="col-form-label" for="tipoliq">Tipo Liquidación</label>
                                            <select class="form-control selected2" id="tipoliq" name="tipoliq" style="height: 36px;" >
                                                <option value="1" {{ old('tipoliq', $tipoliq) == 1 ? 'selected' : '' }}>
                                                    1-Normal
                                                </option>
                                                <option value="2" {{ old('tipoliq', $tipoliq) == 2 ? 'selected' : '' }}>
                                                    2-1era Quincena
                                                </option>
                                                <option value="3" {{ old('tipoliq', $tipoliq) == 3 ? 'selected' : '' }}>
                                                    3-2da Quincena
                                                </option>
                                                <option value="4" {{ old('tipoliq', $tipoliq) == 4 ? 'selected' : '' }}>
                                                    4-SAC
                                                </option>
                                        ...
                                        <div class="col-lg-3 mb-2">
                                            <label class="col-form-label" for="fecha">Fecha liq. *</label>
                                            <div class="input-group" style="width: 220px;">
                                                <input class="form-control"
                                                    ...
                                                    name="fecha"
                                                    autocomplete="off"
                                                    style="height: 36px; padding-left: 12px;"
                                                    value="{{ old('fecha', $fecha ?? date('d/m/Y')) }}"
                                                >
                                                <input type="date" id="fecha_native" style="display:none;">
                                                <button type="button" class="btn btn-outline-secondary" id="btnFecha" style="height: 36px;">
...
<script>
document.addEventListener('DOMContentLoaded', function() {
    const tipoliq = document.getElementById('tipoliq');
    if (!tipoliq) return;

    tipoliq.addEventListener('change', function(e) {
        console.log('tipoliq change ->', e.target.value);
        // Asegurar que la opción seleccionada tenga el atributo selected
        Array.from(tipoliq.options).forEach(opt => {
            opt.selected = (opt.value === e.target.value);
        });
        // Si por alguna razón el control está siendo reemplazado por otra librería,
        // forzamos que el valor del elemento original sea el seleccionado.
        tipoliq.value = e.target.value;
    });
});
</script>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const fechaNative = document.getElementById('fecha_native');
    ...
    if(fechaInput && fechaNative && btnFecha){
        // inicializar native desde el visible
        const initVal = fechaInput.value;
        const ymd = ddmmyyyyToYMD(initVal);
        if(ymd) fechaNative.value = ymd;

        // cuando cambie el native, actualizar formato visible
        fechaNative.addEventListener('change', function(){
            fechaInput.value = ymdToDDMMYYYY(this.value);
        });

        // abrir calendario nativo al hacer click en el botón
